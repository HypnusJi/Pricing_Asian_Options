...
asian_mc_put_cv(2.0,32)
function asian_mc_put_cv(T,m) 
...
fprintf(' n         put(GBM)           call(GBM)         put(BSM)            Variance           stdError             time      \n')
% calculate expectation_Y under the BSM model 
r0=r-sig^2/2; 
sig0=sig*sqrt((2*m+1)/(3*m)); 
T0=(m+1)/(2*m)*T; 
expectation_Y=exp(r0*T0-r*T)*asian_put_bsm(s0*exp(0.5*sig0*sig0*T0),K,sig0,r0,T0); 
for p=1:6 
  n(p)=10*2^(p+10);
end 
s(1)=s0;s2(1)=s0; 
for p=1:6 
    ...
    for j=1:n(p)        
      ...
      %GeometricalBrownianMotion
        ...
        geo_payoff=max(K-ave_g1,0); 
        arith_payoff=max(K-ave_a1,0) ;
...
v2=K*exp(-r*T)*normcdf(-d2)-S*normcdf(-d1);
